...
### **Debug Mode Protection (CRITICAL)**

> #### ⚠️ Warning: Debug Mode Enables Remote Code Execution
>
> Setting `FLASK_DEBUG=1` or `DEBUG=True` in your Superset configuration enables the Werkzeug interactive debugger. This debugger exposes a Python REPL at any unhandled exception endpoint, allowing **arbitrary code execution** on the server — even without authentication. The debugger PIN can be bypassed by an attacker with knowledge of the server environment. **Debug mode must never be enabled in production.**

Superset includes built-in safeguards to prevent debug mode from running in production:

* **Startup check:** If `FLASK_DEBUG=1` is detected alongside `SUPERSET_ENV=production` or a production WSGI server (e.g., gunicorn), Superset logs a `CRITICAL` error and refuses to start.
* **Docker protection:** The production Docker image explicitly sets `FLASK_DEBUG=0`, and the `run-server.sh` entrypoint rejects any attempt to override it.
...
If you need to debug issues in production, use structured logging and a centralized log management system rather than enabling the interactive debugger.
...
#### **Initial Setup:**
  ...
  - [ ] `DEBUG` mode is set to `False` and `FLASK_DEBUG=0` is set in the environment.
  - [ ] `SHOW_STACKTRACE` is set to `False`.
